    ...
    with open(input_filename, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if line.startswith('#'):
                ...
                output_file.append(line)
                if pos == 1 and back_to_toc:
                    output_file.append(
                        f'[{back_to_toc}](#table-of-contents)\n\n')
            else:
                output_file.append(line)
                if line.startswith('```'):
...
def write_back(output_filename, table_of_contents, output_file, placeholder):
    ...
    placeholder += '\n'
    toc_pos = output_file.index(
        placeholder) if placeholder in output_file else 0
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Generate TOC for your Markdown file.')
    parser.add_argument(
        'input_filename',
        metavar='input filename',
        type=str,
        help=
        'The file that need to generate TOC. You must spacify the input filename.'
    )
    parser.add_argument(
        '-o',
        '--output',
    ...
    parser.add_argument(
        ...
        default='~~placeholder~~',
        help=
        'Specify the placeholder, and it will generate TOC at the position of placeholder, otherwise the TOC will generate at the top. NOTE: placeholder="~~placeholder~~" in default.'
    )
    ...
    parser.add_argument(
        ...
        nargs='?',
        default='Back to TOC',
        help='Specify the "back to TOC" anchor under the First level heading.')
...
if __name__ == '__main__':
    ...
    if not output_filename:
        confirm = input(
            'You did not enter output_filename. Would you like to cover your original file with newly generated file? [Y/N]\n'
        )
    ...
    print(
        f'### Generating markdown toc from {input_filename}, and wtite back to {output_filename}...'
    )
    ...
    print(f'### Already generated markdown TOC for {input_filename}.')
